...
# This is a simple function to reshape game frames
...
def saveToCenter(i, rList, jList, bufferArray, summaryLength, h_size, sess, mainQN, time_per_step):
        ...
        for idx, z in enumerate(np.vstack(bufferArray[:,0])):
            img, state_display = sess.run([mainQN.salience, mainQN.rnn_state],\
                                          feed_dict={mainQN.scalarInput:np.reshape(bufferArray[idx,0],[1,21168])/255.0,\
                                                     mainQN.trainingLength:1, mainQN.state_in:state_display, mainQN.batch_size:1})
            imageS.append(img)
        imageS = (imageS - np.min(imageS))/(np.max(imageS) - np.min(imageS))
        imageS = np.vstack(imageS)
        imageS = np.resize(imageS, [len(imageS), 84, 84, 3])
        luminance = np.max(imageS,3)
        imageS = np.multiply(np.ones([len(imageS),84,84,3]),np.reshape(luminance,[len(imageS),84,84,1]))
        make_gif(np.ones([len(imageS),84,84,3]), './Center/frames/sal'+str(i) + '.gif',duration=len(imageS)*time_per_step, true_image=False, salience=True, salIMGS=luminance)
    ...
    with open('./Center/frames/log'+str(i)+'.csv','w') as myfile:
        ...
        a,v=sess.run([mainQN.Advantage,mainQN.Value],\
                     feed_dict={mainQN.scalarInput:np.vstack(bufferArray[:,0])/255.0,mainQN.trainingLength:len(bufferArray),mainQN.state_in:state_train, mainQN.batch_size:1})
        wr.writerows(zip(bufferArray[:,1],bufferArray[:,2],a[:,0],a[:,1],a[:,2],a[:,3],v[:,0]))

# This code allows gifs to be saved of the training episode for use in the Control Center.
def make_gif(images, fname, duration=2, true_image=False, salience=False, salIMAGS=None):
    ...
    def make_mask(t):
        try:
            x = salIMAGS[int(len(salIMAGS)/duration*t)]
        except:
            x = salIMAGS[-1]
        return x
