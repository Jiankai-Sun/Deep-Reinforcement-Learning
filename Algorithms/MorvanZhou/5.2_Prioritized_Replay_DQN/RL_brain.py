...
Reference: https://github.com/MorvanZhou/Reinforcement-learning-with-tensorflow/blob/master/contents/5.2_Prioritized_Replay_DQN/RL_brain.py
...
class SumTree(object):
    ...
    def __init__(self, capacity):
        ...
        self.data = np.zeros(capacity, dtype = object) # for all trasitions
        # [--------------data frame-------------]
        #             size: capacity
...
class Memory(object): # stored as (s, a, r, s_) in SumTree
    ...
    alpha = 0.6 # [0 ~ 1] convert the imporce of TD error to priority
    ...
    def sample(self, n):
        ...
        min_prop = np.min(self.tree.tree[-self.tree.capacity:]) / self.tree.root_priority
        maxiwi = np.power(self.tree.capacity * min_prop, -self.beta) # for later normalizing ISWeights
        for i in range(n):
            a = segment * i
            b = segment * (i + 1)
